...
let base =
  window.location.host.indexOf("localhost") > -1 ? "//localhost:3000/" : "/";
...
var store = new vuex.Store({
  ...
  actions: {
    ...
    // getDisabledDates({ commit, dispatch }) {
    //   api('userparties') // api('boards')
    //     .then(res => {
    //       commit('setDisabledDates', res.data.data)
    //     })
    //     .catch(err => {
    //       commit('handleError', err)
    //     })
    ...
    moveTask({ commit, dispatch }, payload) {
      ...
      api
        .put(`tasks/${payload.task._id}`, payload.task)
        .then(res => {
          dispatch("getTasks", {
            listId: payload.listId,
            boardId: payload.boardId
    ...
    getComments({ commit, dispatch }, payload) {
      //debugger
      api(
        `boards/${payload.boardId}/lists/${payload.listId}/tasks/${
          payload.taskId
        }/comments`
      )
        .then(res => {
    ...
    removeComment({ commit, dispatch }, payload) {
      api
        .delete("comments/" + payload.comment._id)
        .then(res => {
          console.log(res);
          dispatch("getComments", {
            listId: payload.listId,
